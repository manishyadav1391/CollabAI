...
v1 scope: filters at the PROJECT level, using workspace membership and
ProjectPermission. Document-level restriction (the `restricted` flag on
the Document model) is not yet enforced by this function — that's a
known, intentional gap to close before Phase 8's hardening pass finishes.
"""
...
def permission_filtered_project_ids(db: Session, user_id) -> list:
    ...
    workspace_wide_ids = [
        ...
        .filter(Project.workspace_id.in_(workspace_ids), Project.visibility == "workspace_wide")
        .all()
    ]
